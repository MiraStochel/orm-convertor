...
- EF Core parser kromě atributů čte i konvenci primárního klíče: nemá-li entita `[Key]` ani třídní `[PrimaryKey]`, klíčem se stane skalární vlastnost `Id`, jinak `<název entity>Id`, a to bez ohledu na velikost písmen. Klíč z konvence je tvrzení zdroje, ne domněnka, takže má v prioritě zdrojů první stupeň (rozhodnutí [008](./decisions/008-database-as-metadata-source.md)) a v cílovém artefaktu se stává explicitním. Bez toho by entity s konvenčním klíčem — v EF Core běžné — vycházely z převodu jako bezklíčové.
