...
**Generování výstupu** deklaruje abstraktní třída jako jedinou veřejnou abstraktní `Build()` a šest `protected abstract` metod (`BuildImports`, `BuildTableSchema`, `BuildPrimaryKey`, `BuildForeignKey`, `BuildProperties`, `FinalizeBuild`). Skutečnost je ale taková, že tohle rozdělení je pozůstatek staršího návrhu pro jednu entitu: každý konkrétní builder si `Build()` implementuje sám – iteruje přes `EntityMaps` a volá **vlastní privátní statické overloady** týchž metod s parametry `(EntityMap, StringBuilder…)`. Bezparametrické `protected override` verze jsou prázdné stuby s komentářem `// unused in multi-entity flow` — čtyři v Dapper builderu, šest v EF Core a šest v NHibernate. Dapper zbylé dvě, `BuildPrimaryKey` a `BuildForeignKey`, místo toho nechává házet `NotImplementedException`; do `Build()` se nikdy nedostanou. Reálné pořadí kroků je v EF Core `BuildImports` → `BuildTableSchema` → `BuildPrimaryKey` → `BuildProperties` → `BuildForeignKey` → `FinalizeBuild` (primární klíč se generuje **před** vlastnostmi); NHibernate má mezi předposledními dvěma ještě `BuildIdentityMembers`, což je privátní krok mimo šestici abstraktních metod — generuje `Equals`, `GetHashCode` a `[Serializable]` u kompozitního klíče. Kandidát na refaktoring: buď z `Build()` udělat šablonovou metodu nad overloady s parametry, nebo mrtvé stuby odstranit.
