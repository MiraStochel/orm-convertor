...
**Důsledek pro IR.** Zde byl vidět problém, který čekal Java větev. NHibernate má vlastní typovou vrstvu mezi CLR a SQL — `type="string"` není `System.String`. EF Core mapuje CLR typ přímo. Mezireprezentace proto drží obě strany neutrálně: jazykový typ jako `LangType` s uzavřeným seznamem skalárů (rozhodnutí [014](../decisions/014-language-type-model.md)) a databázový typ jako rodinu s facetami unicode, délky a přesnosti (rozhodnutí [019](../decisions/019-neutral-database-type-vocabulary.md)); typová vrstva NHibernate byla pro tu neutrální vrstvu užitečným vzorem. Pět typů, které v uzavřeném seznamu skalárů zprvu chyběly — `DateOnly`, `TimeOnly`, `DateTimeOffset`, `TimeSpan`, pole bajtů —, do něj přidalo rozhodnutí [071](../decisions/071-five-scalars-with-a-counterpart-in-both-ecosystems.md); totéž rozhodnutí zapsalo, že název typu NHibernate je dvojice CLR typ × `DbType` (jeden sloupec `time` má tři jména podle toho, do čeho se čte) a že `TimeSpan` je v EF Core sloupec `time`, kdežto v NHibernate `bigint` s ticky.
...
Není to chyba návrhu. Explicitní junction entita je obecnější — unese payload i kompozitní klíč — a je to jediná forma, kterou umí vyjádřit všechny tři frameworky včetně Dapperu. Proto je v `EntityMap` příznak `IsJunctionTable` jako opt-in signál pro budoucí zploštění na skip navigation. Opačný směr — přečíst skip navigation ze zdroje — umí nástroj z `<many-to-many>` NHibernate, z dvojice kolekčních navigací EF Core (od 2026-09-21; spojovací tabulka je u ní implicitní, takže ji pojmenuje až katalog) a z katalogu samotného. Do analýzy tento případ patří jako doklad rozdílu mezi **expresivitou** (co lze vyjádřit) a **idiomatičností** (jak by to napsal člověk).
...
Výhrada k poslednímu bodu o ADO.NET: v `benchmarks/ORMComparison.sln` to neplatí důsledně a dělicí čára jde čtyři ku třem. Dapper, EF Core, linq2db a RepoDB jedou na `Microsoft.Data.SqlClient` — Dapper a linq2db přes přímou referenci verze 7.0.2, EF Core a RepoDB přes balíky svých SQL Server providerů, takže konkrétní verzi tam určuje provider, ne tenhle soubor. NHibernate (přes `db.Driver<SqlClientDriver>()`), EF6 a PetaPoco jedou na `System.Data.SqlClient` 4.9.1, který k nim ke všem teče přes `benchmarks/Common`; ten sám žádné spojení neotvírá a balík jen podává dál. **PetaPoco byl dosud nedohledaný případ a rozhoduje se u něj vyloučením:** `PetaPoco.Compiled` 6.0.683 nedeklaruje žádnou vlastní závislost na provideru a `Microsoft.Data.SqlClient` se v grafu balíků ani jednoho z obou projektů PetaPoco nevyskytuje, takže `SqlServerDatabaseProvider`, který oba konfigurují, nemá na co jiného než na `System.Data.SqlClient` navázat. Jediná reference čtení kalí, aniž by ho měnila: `EF6Features` jmenuje `Microsoft.Data.SqlClient` přímo, ale žádný kód v tom projektu ho nepoužívá a `EF6Performance`, ze kterého vycházejí naměřená čísla, ho nereferencuje vůbec. Pro srovnání expresivity to nevadí, pro měření výkonu je to metodologický confound.
