...
#### Poddotazy se nevykreslí
*Na řadě. Podklad: audit 2026-08-02, kap. 8. Souvisí s rozhodnutími [023](./decisions/023-query-builder-template-method.md) a [024](./decisions/024-typed-query-operand.md) a s položkou o mezích pravidla 053. Požadavek T2, který dotazovou matici dělí i podle poddotazů.*

Dotazová mezireprezentace zanoření nese, vykreslovací strana ne. `IQueryVisitor` nemá `Visit(SubQueryInstruction)` a `SubQueryInstruction.Accept` vrací prázdný řetězec, takže poddotaz projde parsováním, ale jeho výsledek se nikam neskládá; tiché to není — `Normalize()` v šabloně dotazového builderu (rozhodnutí 023) vnořený poddotaz ohlásí záznamem o ztrátě —, ale vykreslit ho to neumí. Je to poslední kategorie dotazové matice T2, která nemá co měřit: množinové operace i stránkování, které tahle položka původně nesla také, jsou hotové ve všech devíti směrech (rozhodnutí [060](./decisions/060-pagination-as-a-query-instruction.md), `architecture.md` §5).

Zbylá půlka není jen psaní kódu, začíná rozhodnutím o mezireprezentaci. Poddotaz v podmínce — `IN (SELECT …)`, `EXISTS`, porovnání se skalárním poddotazem — potřebuje operand, který typovaný operand rozhodnutí 024 neunese: článek v definici podmínkového stromu operand-poddotaz připouští, model zná jen sloupec a konstantu. K tomu je potřeba říct, ve kterých pozicích se poddotazy vykreslují — zdroj v `FROM`, operand podmínky — a co udělá cíl, který danou pozici nemá.
