...
#### Klíč bez IDENTITY vychází u EF Core jako generovaný
*Nalezeno při implementaci scénářů 4. stupně pro 1:1 a M:N (2026-08-25, [`architecture.md`](./architecture.md) §6.2). Souvisí s rozhodnutími [012](./decisions/012-foreign-key-rendering.md) a [015](./decisions/015-mapping-fact-completion-from-the-catalog.md) a s položkou „Kritérium pro širší čtení konvencí zdroje". Požadavky F4, F6, F11.*

Fáze doplnění nechává strategii klíče, o které zdroj mlčí, u sloupce bez IDENTITY jako `Unspecified` — doplňuje jedině `Identity` tam, kde sloupec IDENTITY je. Oba buildery pak nechají mluvit konvenci cíle, jenže ty dvě konvence tvrdí opak: NHibernate vypíše `assigned` se záznamem o konvenci, kdežto EF Core si u jednodílného celočíselného klíče odvodí generování hodnoty úložištěm. Artefakt nad tabulkou `Products` — klíč přiděluje aplikace — tak tvrdí generování, které schéma nemá, a katalog to věděl. Vidět je to až na 4. stupni, a jen napůl: `DapperManyToManyPersistenceTest` prochází, protože výslovně nastavenou hodnotu klíče EF Core do INSERT vypíše i u vlastnosti považované za generovanou; konzument, který by hodnotu nechal na frameworku, dostane místo přiděleného klíče chybu databáze. Rozhodnout je třeba, kdo má fakt vyslovit. Nabízí se doplňovat `Assigned` symetricky k dnešnímu doplnění `Identity` — kontrola konfliktů už dnes bere `Assigned` nad IDENTITY sloupcem za rozpor, takže by fáze netvrdila nic, co dosud nesoudí —, jenže `ColumnImage` nenese výchozí hodnoty ani sekvence, takže „není IDENTITY" ještě neznamená „přiděluje aplikace"; poctivá odpověď nejspíš vede přes rozšíření čtečky o tenhle fakt. Vypsat `[DatabaseGenerated(None)]` přímo v EF Core builderu u strategie `Unspecified` nejde: bez katalogu by tvrdil přidělování, které nikdo neřekl.
...
*Na řadě. Vyňatá oblast 4 hranice záruk ([`architecture.md`](./architecture.md), §9). Navazuje na rozhodnutí [022](./decisions/022-native-query-syntax-in-builders.md) a [026](./decisions/026-home-of-shared-query-reading.md). Požadavky F7–F10, T2, T3.*
