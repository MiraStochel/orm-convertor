...
Hotová je nově i **generující část F3**: N:M vztah zdroje se před generováním syntetizuje do explicitní junction entity se dvěma vztahy N:1 a kolekce obou stran se přesměrují na ni (rozhodnutí 005, viz `architecture.md`, §4.3); přijetí u obou cílových frameworků soudí nasucho běžící `ManyToManyJunctionVerificationTest`. Parser NHibernate čte i `<key-many-to-one>` — část klíče, která je zároveň odkazem — plochým modelem se záznamem o změněné formě (viz `architecture.md`, §5). A N:M umí najít i **katalog**: spojovací tabulku, kterou žádný artefakt nejmenuje, detekuje fáze doplnění (celý klíč = dva cizí klíče) a přes kolekční navigace zdroje z ní vyvolá touž syntézu; entita namapovaná na spojovací tabulku dostává `IsJunctionTable` (viz `architecture.md`, §5.2, a `DapperManyToManyVerificationTest`).

1. **Doplnění z katalogu — inverzní strana kolekce** (práce) — poslední kus vztahů, který katalog neobsluhuje: inverzní 1:N kolekce (viz níže).
