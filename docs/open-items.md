...
Hotová je nově i **generující část F3**: N:M vztah zdroje se před generováním syntetizuje do explicitní junction entity se dvěma vztahy N:1 a kolekce obou stran se přesměrují na ni (rozhodnutí 005, viz `architecture.md`, §4.3); přijetí u obou cílových frameworků soudí nasucho běžící `ManyToManyJunctionVerificationTest`. Parser NHibernate čte i `<key-many-to-one>` — část klíče, která je zároveň odkazem — plochým modelem se záznamem o změněné formě (viz `architecture.md`, §5). A vztahy z katalogu jsou úplné: vedle vlastnící strany a detekce N:M (spojovací tabulku, kterou žádný artefakt nejmenuje, najde fáze doplnění a přes kolekční navigace zdroje z ní vyvolá syntézu; entita namapovaná na spojovací tabulku dostává `IsJunctionTable`) obsluhuje fáze doplnění nově i **inverzní stranu kolekce** — existující 1:N dostane páry `<key>` sloupců z cizího klíče podřízené tabulky a kolekční navigace bez vztahu (Dapper zdroj) dostane vztah syntetizovaný, kolekce sama se nikdy nevymýšlí (viz `architecture.md`, §5.2, a `CatalogInverseCollectionTest`; proti databázi `DapperManyToManyVerificationTest` a kolekce ve scénářích `DapperTo*VerificationTest`).

1. **Klíčová třída u formy `Embedded`** (rozhodnutí) — přijetí frameworkem ji už umí odhalit: u té formy nejsou části klíče vlastnostmi entity, takže mapování odkáže na vlastnost, kterou třída nemá, a 3. stupeň takový pár odmítne. Tvarová aserce to odhalit nemohla.
2. **Zbytek** podle priorit vyplývajících z požadavků F/S/T.
