...
Rozhodnout je třeba, jestli je hranicí pravidla *podmínka* (dnešní znění 053), nebo *množina řádků* (jeho vlastní argument). Druhá odpověď je přísnější a konzistentnější, ale není zadarmo: u joinu by znamenala nevydat artefakt tam, kde ho dnes uživatel dostane spolu se srozumitelným záznamem o tom, co se stalo, a matice T2 by v kategorii *druh joinu* přestala měřit překlad a začala měřit odmítnutí. Volba se navíc dotýká čtení T3: dnes se takový výstup počítá jako přeložený a funkčně neekvivalentní, po zpřísnění by se nepočítal vůbec. Poddotazová půlka se překrývá s položkou *Poddotazy a stránkování se nevykreslí* — až se poddotazy vykreslovat začnou, přestane být náhradou a otázka zbude jen u joinu.

### Práce

#### Vztahy 1:1 a M:N nemají spuštěný scénář 4. stupně
*Potom. Zbytek kritéria F3 po prvních zástupcích 4. stupně (2026-08-24, [`architecture.md`](./architecture.md) §6.2); stupně podle rozhodnutí [016](./decisions/016-generated-artifact-verification-levels.md). Požadavky F3, F4, F6.*

Čtvrtý stupeň ověření — entita se uloží a načte se stejnou identitou — má zástupce pro vztah 1:N: `DapperToNHibernatePersistenceTest` a `DapperToEFCorePersistenceTest` uloží zákazníka s objednávkami přes vygenerované artefakty, načtou ho zpět i s kolekcí a NHibernate půlka předvádí negativně kolekci deklarovanou konkrétním typem (třída vad rozhodnutí [035](./decisions/035-nhibernate-collections-declared-by-interface.md)). Kritérium F3 ale žádá spuštěný test pro **každý** typ vztahu a 1:1 s M:N zatím končí na 3. stupni. Schéma fixture oba případy nese (`CustomerProfiles` se sdíleným klíčem, `ProductSuppliers` jako spojovací tabulka) a M:N je proti databázi přijaté 3. stupněm (`DapperManyToManyVerificationTest`); zbývá u obou týž scénář uložení a načtení, jaký má 1:N — u M:N včetně průchodu přes syntetizovanou junction entitu.
...
#### Poddotazy a stránkování se nevykreslí
*Na řadě. Podklad: audit 2026-08-02, kap. 8. Souvisí s rozhodnutími [023](./decisions/023-query-builder-template-method.md) a [024](./decisions/024-typed-query-operand.md) a s položkou o mezích pravidla 053. Požadavek T2, který dotazovou matici dělí i podle poddotazů a stránkování.*

Dotazová mezireprezentace zanoření nese, vykreslovací strana ne. `IQueryVisitor` nemá `Visit(SubQueryInstruction)` a `SubQueryInstruction.Accept` vrací prázdný řetězec, takže poddotaz projde parsováním, ale jeho výsledek se nikam neskládá; tiché to není — `Normalize()` v šabloně dotazového builderu (rozhodnutí 023) vnořený poddotaz ohlásí záznamem o ztrátě —, ale vykreslit ho to neumí. Stránkování mezireprezentace nenese vůbec a parsery ho hlásí jako ztrátu, od 2026-08-25 včetně `TOP` a `OFFSET/FETCH` na straně SQL, které dřív mizely beze stopy. Obojí jsou kategorie dotazové matice podle T2, které tak nemají co měřit; množinové operace, které tahle položka původně nesla také, jsou hotové ve všech devíti směrech (`architecture.md` §5).

Ani jedna zbylá půlka už není jen psaní kódu, obě začínají rozhodnutím o mezireprezentaci. Poddotaz v podmínce — `IN (SELECT …)`, `EXISTS`, porovnání se skalárním poddotazem — potřebuje operand, který typovaný operand rozhodnutí 024 neunese: článek v definici podmínkového stromu operand-poddotaz připouští, model zná jen sloupec a konstantu. A stránkování není v normalizované sadě instrukcí článku vůbec, takže jde o rozšíření slovníku instrukcí nad jeho rámec — s otázkami, které rozšíření nese s sebou: jak se stránkování váže na řazení (T-SQL `OFFSET/FETCH` bez `ORDER BY` nezapíše, LINQ `Skip`/`Take` bez řazení vrací nedeterministický výřez) a kde se vykreslí u cíle, jehož jazyk je nenese v textu. U poddotazů je k tomu potřeba říct, ve kterých pozicích se vykreslují — zdroj v `FROM`, operand podmínky — a co udělá cíl, který danou pozici nemá.
