...
Tvrzení o frameworcích platí pro NHibernate 5.7.0 a EF Core 10.0.10; u JPA se opíráme o Jakarta Persistence, kde hodnota `UUID` přibyla ve verzi 3.1. Cílové verze zafixovalo den po tomto rozhodnutí [013](013-target-framework-versions.md) na Jakarta Persistence 3.2; volba slovníku se tím nemění.
...
**Název výčtu `PrimaryKeyStrategy` zůstává.** Přesnější `KeyGenerationStrategy` zamítáme kvůli papírové stopě: nese ho kategorie `MappingFactCategory.PrimaryKeyStrategy` v deskriptoru, opisem se na něj odvolává rozhodnutí [006](006-flat-composite-key-rendering.md) a jmenuje ho audit 2026-08-02 (nález 4.7) — a rozhodnutí se nepřepisují. Že jde o strategii části, říká už umístění — `PrimaryKeyPart.Strategy`.
...
Parametry generátoru tím dostávají místo v modelu, ne zdroj. Kde je zdroj neuvádí, zůstanou prázdné; doplnění z databázového katalogu (rozhodnutí 015) je samostatná práce a týká se hlavně identity sloupce a výchozí hodnoty odkazující na sekvenci.
...
**2026-08-18 — revidováno.** Opraveny dvě nepřesnosti, které našel audit 2026-08-15. Papírová stopa u názvu výčtu jmenovala rozhodnutí 006 a 009; identifikátor `PrimaryKeyStrategy` ale v žádném z nich není — 006 má jen opis a 009 strategii generování klíče nezmiňuje —, takže citace teď vede na kategorii v deskriptoru a na audit (nález 2.2). A podklad se opíral o Jakarta Persistence 3.1 s poznámkou, že deklarace cílových verzí je otevřená položka; ta se den nato uzavřela rozhodnutím 013 na verzi 3.2 (nález 2.4). Volba se ani v jednom případě nemění, opravená je jen opora, o kterou se text opíral.
