...
/// <summary>
/// Emits T-SQL through the steps of the template (decision 023): clause order,
/// placement of TOP and OFFSET/FETCH, set operations, subqueries and the result type
/// derived from the table. Everything here is a property of the language, so it is shared
/// by every framework whose queries are SQL (decision 082) - what is framework-specific is
/// the single step that wraps a finished SELECT into artifacts, and that is
/// <see cref="Emit"/>, which the wrapper supplies.
/// </summary>
public abstract class AbstractSqlQueryBuilder : AbstractQueryBuilder
{
    private readonly IQueryVisitor visitor;

    protected AbstractSqlQueryBuilder()
    {
    ...
    protected override void BuildSource(QueryClauses clauses, QueryArtifact artifact)
    {
        ...
        // A framework whose queries are SQL materializes into a type the query itself never
        // names - Dapper's Query<T>, a MyBatis resultType. The entities of the
    ...
    /// <summary>
    /// The one step a framework whose queries are SQL does not share: how a finished SELECT
