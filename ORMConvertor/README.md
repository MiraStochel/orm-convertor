...
**Verified by running it on two machines**, on 2026-08-23 and on 2026-08-24, each time over the whole of the suite as it stood that day. `docker compose --profile test run --rm tests` ran from a clean state and finished **with no failure and no skip**; the zero skips are the significant number, because that is what separates a run which demonstrated criteria F4 and F6 from one which merely did not contradict them. Both enforcement branches were tried as well: with the variable set and the database unreachable the suite fails with the stated reason, and without the variable and without a connection string it skips and stays green. The second machine ran the `build`-then-`run` pair above and its count came out **equal to that machine's own host run of the same tree**, which is the staleness check passing rather than merely being described.
...
**Measured on 2026-08-24** (Release, host with a local SQL Server, connection string in user secrets), with `dotnet test Tests/Tests.csproj --configuration Release --collect:"XPlat Code Coverage"`: **484 passed, 0 failed, 0 skipped**. Coverage **70.8 % of lines and 58.1 % of branches** overall; of the individual projects the highest are `SampleData` and `CSharpEntityParsing` (100 %), `OrmConvertor` (95.0 %) and `Model` (93.3 %), the lowest **`Advisor` and `AdvisorBenchmarking`, both at 0.0 %** — which makes the exclusion of area 1 in [`architecture.md`](../docs/architecture.md) §9 a matter of a measured figure rather than of reading — and after them `ORMConvertorAPI` (42.4 %). That last figure is new: before the tests over HTTP (decision [043](../docs/decisions/043-rest-contract-guarded-over-http.md), [`architecture.md`](../docs/architecture.md) §6.5) it was **6.0 %**, because the endpoint handlers were exercised only through the orchestration. Taken on its own it is also lower than the project's own code looks: `Program` is at 100 %, `Endpoints` at 78.3 %, and all the DTOs and the data in `Data/` at 100 %. Two things pull the average down — types generated into the assembly by the `Microsoft.AspNetCore.OpenApi` source generator (XML comment support, not our code) and the Advisor handlers with their DTOs at 0 %, i.e. excluded area 1.

**A second machine measured the same commit the same day and did not get the same totals.** It reported **507 passed, 0 failed, 0 skipped** and **71.3 % of lines, 58.9 % of branches**, and its containerized run agreed on 507 independently of its host run. What did *not* differ is everything else: each per-project figure named above came back identical — `SampleData` and `CSharpEntityParsing` at 100 %, `OrmConvertor` at 95.0 %, `Model` at 93.3 %, `Advisor` and `AdvisorBenchmarking` at 0.0 %, `ORMConvertorAPI` at 42.4 % — so the disagreement is confined to the total, and the last commit touching `Tests/` is older than both runs. Which count describes this tree is therefore an open question rather than a machine difference, and until it is answered this section carries both numbers instead of quietly preferring one; the item is in [`open-items.md`](../docs/open-items.md). Nothing else in the repository repeats either figure, so there is no second place to drift.
