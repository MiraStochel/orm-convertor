...
**Covered:** entity, mapping, and query translation as described above, Hibernate and EclipseLink included, merging of multi-file input into one conversion with per-file input and output, completion from the database catalog, structured diagnostics of every conversion — stamped with a run identifier and the versions of the tool and of both frameworks — deterministic output, translation artifacts that never carry database credentials, translation performance measured against a stated limit, an architecture in which a new framework is a new self-contained wrapper, and a documented container configuration that both runs the system and reproduces both test suites, .NET and Java, on a machine with nothing installed but Docker.

Five of those hold in a narrower sense than the words alone suggest:

- Hibernate and EclipseLink are covered as two frameworks, not as the Java ecosystem. Generated artifacts for each are compiled with `javac`, accepted by that implementation and run against SQL Server by the Java test suite, in three source directions apiece; MyBatis, translation across the whole framework matrix and differential verification of query results are not claimed at all. A Java target enters the guarantees only once its artifacts have really been compiled and accepted in the pinned environment, which is not a formality: that run refuted one assumption about Hibernate and corrected three about EclipseLink. One thing is not claimed for EclipseLink even so — lazy loading of a reference: `fetch = LAZY` on a `@ManyToOne` or `@OneToOne` is silently eager there unless the consumer project weaves its bytecode, and the intermediate representation carries no loading strategy, so the generated artifact does not state one and a source that did gets a record.
...
- **Next: the Java ecosystem.** Hibernate, MyBatis, and EclipseLink, cross-ecosystem translation between .NET and Java, and a Java test infrastructure that verifies query results differentially. Hibernate and EclipseLink are in place and claimed, together with the Java test suite that compiles, accepts and runs what the tool generates for each of them; MyBatis is what comes next. This is the milestone the next MAJOR is reserved for, and apart from those two frameworks none of it is claimed today.
