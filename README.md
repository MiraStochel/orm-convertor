...
The tag `1.0` predates this policy and does not contain the container configuration described above, although its own guarantees section already claimed it. Under the rule just stated it is not moved: the release `1.1.0` corrects it instead, and is a MINOR step for a single reason — an area entered the boundary above, while the generated artifacts and the REST contract are unchanged.
