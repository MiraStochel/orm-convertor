...
- Hibernate is covered as one framework, not as the Java ecosystem. Generated Hibernate artifacts are compiled with `javac`, accepted by a Hibernate session factory and run against SQL Server by the Java test suite, in three source directions; MyBatis, EclipseLink, translation across the whole framework matrix and differential verification of query results are not claimed at all.
