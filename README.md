...
The measured output of that comparison is **not versioned** ([decision 042](docs/decisions/042-measured-benchmark-output-out-of-git.md)): it is a single run from March 2025, and the charts in it are re-rendered from the measurements by the R script that ships beside them. A clone therefore contains the benchmark projects and their description, not their results; earlier revisions still hold them, so `git show <commit>:benchmarks/results/…` retrieves any of it.
